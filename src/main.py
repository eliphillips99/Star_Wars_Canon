...
def main():
    ...
    if mode == 1:  # Compare to integer
        ...
        results_df = process_all_entries(df, API_KEYS["tmdb"], API_KEYS["gemini"], items_to_process=items_to_process, process_gemini=True)

        # Save results to a CSV
        save_results_to_csv(results_df, "data/Timeline_with_TMDb.csv")
    elif mode == 2:  # Compare to integer
        input_csv = "data/Timeline_with_TMDb.csv"
